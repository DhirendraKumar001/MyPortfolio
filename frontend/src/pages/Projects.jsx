...
export default function Projects() {
  ...
  if (loading) return (
    <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6ee7b7' }}>
      Loading projects...
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '4rem 2rem' }}>
      ...
      <p style={{ color: '#64748b', marginBottom: '3rem' }}>Things I've built and shipped</p>
      ...
      {projects.length === 0 ? (
        <div style={{
          ...
          No projects yet. Add them via the admin API.
        </div>
      ) : (
        <div style={{
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginBottom: '1.5rem' }}>
                  {project.techStack?.split(',').map(tech => (
                    <span key={tech} style={{
                      ...
                      color: '#818cf8', borderRadius: 6, padding: '0.25rem 0.6rem', fontSize: '0.75rem'
                    }}>
                      {tech.trim()}
                    </span>
                  ))}
                ...
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  {project.githubUrl && (
                    <a href={project.githubUrl} target="_blank" rel="noreferrer" style={{
                      color: '#6ee7b7', fontSize: '0.85rem', textDecoration: 'none',
                      border: '1px solid rgba(110,231,183,0.3)', borderRadius: 8, padding: '0.4rem 1rem'
                    }}>
                      GitHub →
                    </a>
                  )}
                  {project.liveUrl && (
                    <a href={project.liveUrl} target="_blank" rel="noreferrer" style={{
                      ...
                      textDecoration: 'none', borderRadius: 8, padding: '0.4rem 1rem'
                    }}>
                      Live Demo
                    </a>
                  )}
